...
from datalake_conts import SERVICE_ENROLLMENT_TABLE, SUPPLEMENTARY_NUTRITION_FORM_TABLE, GROWTH_MONITORING_FORM_TABLE, CHILD_THR_FORM_TABLE
...
class AggChildCareMonthly(BaseAggregationHelper):
    ...
    def source_tablename(self):
        return f"{clean_name(self.database_name)}.child_care_case"

    @property
    def member_case_table(self):
        return f"{clean_name(self.database_name)}.member_case"

    @property
    def location_table(self):
        return f"{clean_name(self.database_name)}.flwc_location"

    @property
    def service_enrollment_table(self):
        return f"{clean_name(self.database_name)}.{SERVICE_ENROLLMENT_TABLE}"

    @property
    def snd_table(self):
        return f"{clean_name(self.database_name)}.{SUPPLEMENTARY_NUTRITION_FORM_TABLE}"

    @property
    def gm_table(self):
        return f"{clean_name(self.database_name)}.{GROWTH_MONITORING_FORM_TABLE}"

    @property
    def thr_table(self):
        return f"{clean_name(self.database_name)}.{CHILD_THR_FORM_TABLE}"
    ...
    def aggregate_data(self, df):
        ...
        born_in_month = (f"(member_case.case_json__member_dob>={self.month} AND "
                         f"member_case.case_json__member_dob<add_months('{self.month}', 1))")
